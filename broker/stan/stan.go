...
import (
	...
	"github.com/laoqiu/go-plugins/broker/codec/json"
...
func (n *subscriber) Unsubscribe() error {
	...
	// server Unsubscribe can't handle durable queues, so close as stan suggested
...
func (n *stanBroker) connect() error {
	...
	for {
		...
		case <-ticker.C:
			err := fn()
			if err == nil {
				glog.Infof("[stan]: successeful connected to %v", n.addrs)
				return nil
			}
			glog.Errorf("[stan]: failed to connect %v: %v\n", n.addrs, err)
		}
	}
...
// NewBroker return new stanBroker
